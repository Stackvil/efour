...
const PolicyConsent = () => {
    ...
    useEffect(() => {
        // Updated key to ensure users see the simplified version
        const hasAccepted = localStorage.getItem('efour-policy-v3-accepted');
        if (!hasAccepted) {
    ...
    const handleAccept = () => {
        localStorage.setItem('efour-policy-v3-accepted', 'true');
        setIsVisible(false);
    };
